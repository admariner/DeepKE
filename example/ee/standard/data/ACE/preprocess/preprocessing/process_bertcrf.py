...
def read_by_lines(path):
        ...
        with open(path, "r", encoding="utf8") as infile:
                result.extend(line.strip() for line in infile)
        return result
...
def data_process(raw_data, task="trigger", mode="train"):
        ...
        for idx, line in enumerate(tqdm(raw_data, desc=f"processing {mode} {task}")):
                ...
                if task == "trigger":
                        ...
                        output.append(f'{" ".join(text_a)}\t{" ".join(trigger_labels)}\t{str(idx)}')
                elif task == "role":

                                    # for negative samples
                        if len(line["event_mentions"]) == 0:
                                output.append(
                                    f'{" ".join(text_a)}\t{" ".join(["O"] * len(text_a))}\t{" ".join(["O"] * len(text_a))}\t{str(idx)}'
                                )
                                continue
                        ...
                        for mention in line["event_mentions"]:
                                ...
                                output.append(
                                    f'{" ".join(text_a)}\t{" ".join(role_labels)}\t{" ".join(trigger_labels)}\t{str(idx)}'
                                )
...
def main():
        ...
        for fold in ["train", "dev", "test"]:
                print(os.path.join(args.base_path, f"{fold}.w1.oneie.json"))
                raw_data = open(
                    os.path.join(args.base_path, f"{fold}.w1.oneie.json"),
                    "r").readlines()
                ...
                write_by_lines(os.path.join(trigger_path, f"{fold}.tsv"), processed_trigger)
                write_by_lines(os.path.join(role_path, f"{fold}.tsv"), processed_role)
